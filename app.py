...
def set_background(svg_path: str = "assets/bg.svg"):
    """Set a page background using a local SVG file (embedded as base64) or fall back to a gradient."""
    p = Path(svg_path)
    if p.exists():
        svg = p.read_text(encoding='utf8')
        ...
        /* Make the main container slightly translucent so content stands out */
        .stApp > .main {{
        ...
        st.markdown(
            """
            <style>
            .stApp {background: linear-gradient(180deg, #f7f9fb 0%, #eef3f7 100%);} 
            .stApp > .main {background-color: rgba(255,255,255,0.9);} 
            </style>
            """,
